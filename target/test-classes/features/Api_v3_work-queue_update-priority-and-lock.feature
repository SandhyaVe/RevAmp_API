Feature: Test Api End Point - /v3/work-queue/update-priority-and-lock
 ...
 * def encryptedBody = crypto.encrypt(requestBodyStr)
 ...
 Given path '/work-queue-service/v3/work-queue/update-priority-and-lock'
 ...
 * match decryptedResponse.message == '#regex Work queue updated successfully\\.'
...
//  "lockType":"LOCK"
  ...
  * def encryptedBody = crypto.encrypt(requestBodyStr)
  ...
  Given path '/work-queue-service/v2/work-queue/update-priority-and-lockhhhh'
  And header Cookie = 'auth_token=' + authCookie
  And header X-ECDH-Session = ecdhSessionId
  And header X-Encrypted-Payload = 'true'
  And header Content-Type = 'text/plain'
