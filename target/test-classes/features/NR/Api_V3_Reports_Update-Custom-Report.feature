Feature: Test Api end point - /v3/reports/update-custom-report

Background:
  * url 'https://apiqa.revamprcm.com'
  * def mainToken = karate.get('authToken')
  #* call read('classpath:features/Set-Client_For_Api_End_Points.feature')


 Scenario: Positive Flow
 * def requestBody =
       """
      {
        "reportId": 164,
        "statement": "Select",
        "condition": "",
        "groupBy": "",
        "isUnique": false,
        "tableName": "vw_patient",
        "fieldId": "187,189"
      }
       """
 Given path '/v3/reports/update-custom-report'
 And header Authorization = 'Bearer ' + mainToken
 And request requestBody
 When method post
 Then status 200
 * print response
 * match response.message contains 'Custom report updated successfully.'