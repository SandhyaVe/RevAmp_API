Feature: Test Api End Point - /v3/custom-reports/get-report-information
 ...
 * def encryptedBody = crypto.encrypt(requestBodyStr)
 ...
 Given path '/report-service/v3/custom-reports/get-report-information'
 ...
 * match decryptedResponse.reportFields == '#[]'
 * match each decryptedResponse.reportFields ==
 """
  {
    "AutoID": '#number',
    "field_name": '#string',
  ...
  * match decryptedResponse.reportConditions == '#[]'
  * match each decryptedResponse.reportConditions ==
   """
   {
    "field_Name": '#string',
  ...
  * def encryptedBody = crypto.encrypt(requestBodyStr)
  ...
  Given path '/report-service/v2/custom-reports/get-report-informationuuu'
  And header Cookie = 'auth_token=' + authCookie
  And header X-ECDH-Session = ecdhSessionId
  And header X-Encrypted-Payload = 'true'
  And header Content-Type = 'text/plain'
