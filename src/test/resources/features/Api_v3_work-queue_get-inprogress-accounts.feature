 ...
 * def encryptedBody = crypto.encrypt(requestBodyStr)
 ...
 Given path '/work-queue-service/v3/work-queue/get-inprogress-accounts'
 ...
 * match each decryptedResponse.accountsData ==
  """
  {
   "row_number": '#number',
  ...
  * def encryptedBody = crypto.encrypt(requestBodyStr)
  ...
  Given path '/work-queue-service/v2/work-queue/get-inprogress-accountssss'
  And header Cookie = 'auth_token=' + authCookie
  And header X-ECDH-Session = ecdhSessionId
  And header X-Encrypted-Payload = 'true'
  And header Content-Type = 'text/plain'
